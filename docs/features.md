...
- **Supply Manager integration.** The Review modal now requires a funding source before approve unless the reviewer explicitly toggles "funding not required". The selected source's grant identifier and validity-window badge are shown next to the picker, with an inline warning when the source is within 30 days of expiration. The request card's funding line shows the assigned source + grant id + status pill to admins (with a Change-source dropdown and Clear button while the request is `submitted` / `approved` / `ordered`); members see only "Funding: assigned by reviewer — {nickname}" with no grant id.
